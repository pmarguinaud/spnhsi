SUBROUTINE SPCIMPFSOLVE(&
 ! --- INPUT -----------------------------------------------------------------
 & YDGEOMETRY,YDRIP,YDDYN,KMLOCSTA,KMLOCEND,KSTA,KEND,SIVP,PSDIVP,&
 ! --- OUTPUT ----------------------------------------------------------------
 & PSPDIVP)  
...
REAL(KIND=JPRB)   ,INTENT(IN)    :: SIVP (YDGEOMETRY%YRDIMV%NFLEVG)
REAL(KIND=JPRB)   ,INTENT(INOUT)    :: PSDIVP (KSTA:KEND,YDGEOMETRY%YRDIMV%NFLEVG)
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PSPDIVP(KSTA:KEND,YDGEOMETRY%YRDIMV%NFLEVG)
...
real(kind=jprb), allocatable :: zsdivpinter(:,:),zspdivpinter(:,:)
...
!*        1.2  Main calculations.
  ...
  IF (KMLOCSTA==KMLOCEND) THEN
    ...
    CALL SIMPLICO(IM,NSMAX,YDGEOMETRY%YRDIMV%NFLEVG,YDGEOMETRY%YRDIMV%NFLEVG,ZALPHA(IM),&
     & ZDENIM(IM),ZFPLUS(IM),ZFMINUS(IM),SIVP,YDLAP%RLAPDI(0:NSMAX),&
     & ZBDT2,PSDIVP,PSPDIVP)

  ELSE

    CALL SIMPLICO(IM,NSMAX,YDGEOMETRY%YRDIMV%NFLEVG,YDGEOMETRY%YRDIMV%NFLEVG,ZALPHA(IM),&
     & ZDENIM(IM),ZFPLUS(IM),ZFMINUS(IM),SIVP,YDLAP%RLAPDI(0:NSMAX),&
     & ZBDT2,PSDIVP(ISTA:IEND,:),PSPDIVP(ISTA:IEND,:))
