...
INTEGER(KIND=JPIM) :: ISPCOL, JLEV, JSP, JN, IOFF
...
#include "spcsidg_part0nhtot.intfb.h"
#include "spcsidg_part1tot.intfb.h"
#include "spcsidg_part2tot.intfb.h"
...
!$OMP END PARALLEL DO
  ...
  IF (KM==-999) THEN
    CALL SPCSIDG_PART2TOT(YDGEOMETRY,KSTA,KEND,ZR1D,ZR1D2,&
...
!$OMP END PARALLEL DO
    IF (LSIDG) THEN 
      ...
      IF (KM==-999) THEN 
        CALL SPCSIDG_PART2TOT(YDGEOMETRY,KSTA,KEND,ZR3D,ZR4D,&
      &KM,KMLOC,LSIDG,KSZNISNAX,NISNAX,SCGMAP(:,1),SCGMAP(:,2),SCGMAP(:,3))
      ELSE
...
!$ACDC }
  ...
  IF (LSIDG .OR. LESIDG) THEN
    ...
    ! Use ZSDIVPL and ZSPDIVPL as intermediate work arrays.
  IF (KM==-999) THEN
    CALL SPCSIDG_PART1TOT(YDGEOMETRY,YDDYN,KSTA,KEND,ZSDIVP,&
         &ZSPDIVP,KM,KMLOC,LSIDG,KSZNISNAX,NISNAX,SIHEGTRA,SIHEG2TRA)
  
    CALL SPCSIDG_PART0NHTOT(YDGEOMETRY,YDDYN,KSTA,KEND,ZSPDIVP,&
     & PLAPIN,KM,IOFF)
...
!$OMP END PARALLEL DO
  ...
  IF (LSIDG .OR. LESIDG) THEN
    ...
    !   Use ZSDIVPL and ZSPDIVPL as intermediate work arrays.
  IF (KM==-999) THEN
    CALL SPCSIDG_PART0NHTOT(YDGEOMETRY,YDDYN,KSTA,KEND,ZSRHS,&
     & PLAPIN,KM,IOFF)
  
    CALL SPCSIDG_PART1TOT(YDGEOMETRY,YDDYN,KSTA,KEND,ZSRHS,&
...
!$OMP END PARALLEL DO
  ...
  IF (LSIDG) THEN
    ...
    ! Use ZSDIVPL and ZSPDIVPL as intermediate work arrays.
    IF (KM==-999) THEN
      CALL SPCSIDG_PART2TOT(YDGEOMETRY,KSTA,KEND,ZZSPDIVG,ZWORK,&
        &KM,KMLOC,LSIDG,KSZNISNAX,NISNAX,SCGMAP(:,1),SCGMAP(:,2),SCGMAP(:,3))
    ELSE
